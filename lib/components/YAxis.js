...
var YAxis = function (_React$Component) {
    ...
    _createClass(YAxis, [{
        ...
        value: function postSelect(style, hideAxisLine, height) {
            ...
            (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").selectAll(".tick").select("text");
            // .styles(valueStyle);

            (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").selectAll(".tick").select("line");
            // .styles(tickStyle);

            (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").selectAll(".domain").remove();

            if (!hideAxisLine) {
                (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").append("line")
                // .styles(axisStyle)
        ...
        value: function renderAxis(align, scale, width, height, showGrid, chartExtent, hideAxisLine, absolute, type, fmt, label, tickCount, min, max) {
            ...
            this.axis = (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).append("g").attr("transform", "translate(" + x + ",0)").attr("class", "yaxis")
            // .styles(valueStyle)
            .call(axisGenerator.tickSize(tickSize)).append("text").text(label || this.props.label)
            // .styles(labelStyle)
            .attr("transform", "rotate(-90)").attr("class", "yaxislabel").attr("y", labelOffset).attr("dy", ".71em").attr("text-anchor", "end");
